...
export default function EditCleanerPage() {
  ...
  useEffect(() => {
    async function loadCleaner() {
      try {
        const res = await fetch(`/cleaning/api/cleaners/${id}`);
        ...
        setFormData({
          ...
          email: cleaner.email || "",
          phone: cleaner.phone || "",
          specializations: cleaner.specializations || [],
          hourlyRate: cleaner.hourlyRate || 0,
          zone: cleaner.zone || "",
          homeLocation: {
            address: cleaner.homeLocation?.address || "",
            city: cleaner.homeLocation?.city || "",
            state: cleaner.homeLocation?.state || "",
            zipCode: cleaner.homeLocation?.zipCode || "",
          },
          serviceRadius: cleaner.serviceRadius || 10,
          schedulePreference: {
            preferredStartTime:
              cleaner.schedulePreference?.preferredStartTime || "08:00",
            preferredEndTime:
              cleaner.schedulePreference?.preferredEndTime || "17:00",
            maxHoursPerWeek:
              cleaner.schedulePreference?.maxHoursPerWeek || 40,
          },
          notes: cleaner.notes || "",
        });
      } catch {
        toast.error("Failed to load cleaner data");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch(`/cleaning/api/cleaners/${id}`, {
        method: "PUT",
      ...
      router.push(`/dashboard/cleaners/${id}`);
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Failed to update cleaner"
      );
    } finally {
    ...
    <div className="space-y-6">
      <PageHeader title="Edit Cleaner">
        <Button variant="outline" size="sm" asChild>
          <Link href={`/dashboard/cleaners/${id}`}>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  type="time"
                  value={
                    formData.schedulePreference?.preferredStartTime || "08:00"
                  }
                  onChange={(e) =>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  type="time"
                  value={
                    formData.schedulePreference?.preferredEndTime || "17:00"
                  }
                  onChange={(e) =>
                    updateSchedulePreference(
                      "preferredEndTime",
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  max="168"
                  value={
                    formData.schedulePreference?.maxHoursPerWeek || 40
                  }
                  onChange={(e) =>
                    updateSchedulePreference(
                      "maxHoursPerWeek",
        ...
        <div className="flex items-center justify-end gap-3">
          <Button type="button" variant="outline" asChild>
            <Link href={`/dashboard/cleaners/${id}`}>Cancel</Link>
          </Button>
          <Button
            type="submit"
